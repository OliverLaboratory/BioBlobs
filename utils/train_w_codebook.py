...
class MultiStageBioBlobsLightning(pl.LightningModule):
    ...
    def setup_stage(self, stage_idx: int):
        ...
        # Get stage configuration from train_cfg
        stage_cfg = getattr(self.train_cfg, f"stage{stage_idx}")
        ...
        print(
            f"✓ Loss weights: λ_vq={self.model.lambda_vq:.1e}, λ_ent={self.model.lambda_ent:.1e}, λ_psc={self.model.lambda_psc:.1e}"
        )
        print(f"{'=' * 60}\n")
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_vq_loss", vq_loss, on_step=True, on_epoch=True, batch_size=batch_size
        )
        self.log("train_acc", acc, on_step=True, on_epoch=True, batch_size=batch_size)
        self.log(
            "stage",
            float(self.current_stage),
            on_step=True,
            on_epoch=True,
    ...
    def on_train_epoch_start(self):
        ...
        if self.loss_weight_scheduler is not None:
            self.current_loss_weights = self.loss_weight_scheduler.get_weights(
                self.stage_epoch
            )
            self.model.lambda_vq = self.current_loss_weights.get("lambda_vq", 0.0)
            self.model.lambda_ent = self.current_loss_weights.get("lambda_ent", 0.0)
        ...
        if self.freeze_codebook_remaining_epochs > 0:
            ...
            if (
                epochs_in_stage
                >= total_stage_epochs - self.freeze_codebook_remaining_epochs
            ):
                # Freeze codebook
                ...
                if (
                    ...
                    print(
                        f"🔒 Codebook frozen for final {self.freeze_codebook_remaining_epochs} epochs"
                    )
    ...
    def on_train_epoch_end(self):
        ...
        print(
            f"[{stage_name}] Epoch {self.stage_epoch - 1:3d} | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.4f}"
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "val_vq_loss", vq_loss, on_step=False, on_epoch=True, batch_size=batch_size
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log(
            "test_vq_loss", vq_loss, on_step=False, on_epoch=True, batch_size=batch_size
        )
    ...
    def get_inter_info(
        ...
        if self.bypass_codebook:
            print(
                "⚠️ Warning: Interpretability analysis not available in bypass_codebook mode"
            )
            return None
    ...
    def configure_optimizers(self):
        ...
        if self.train_cfg.get("use_cosine_schedule", False):
            ...
            scheduler = get_cosine_schedule_with_warmup(
                optimizer, warmup_epochs, stage_epochs
            )
            return {
                "optimizer": optimizer,
                "lr_scheduler": {
