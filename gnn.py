...
class GINConv(gnn.MessagePassing):
    def __init__(self, embed_dim=256, use_edge_attr=False, edge_attr_dim=1):
        ...
        if use_edge_attr:
            self.edge_encoder = nn.Linear(edge_attr_dim, embed_dim)
...
class GNN(nn.Module):
    def __init__(self, embed_dim=256, num_layers=3, dropout=0.0, gnn_type='gin',
                 use_edge_attr=False, pe=None, edge_attr_dim=1):
        super().__init__()
        ...
        for _ in range(num_layers):
            if gnn_type == 'gin':
                self.gnns.append(gnn_model(embed_dim, use_edge_attr=use_edge_attr, edge_attr_dim=edge_attr_dim))
            else:
